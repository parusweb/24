...
function updateMultiplierCalc() {
    ...
    if (selectedShapeInput) {
        ...
        shapeExtraPerItem = length_m * unitShapePrice * priceMultiplier;
    ...
    if (shapeExtraTotal > 0) {
        html += `Цена выбранной верхушки (за все): <b>${shapeExtraTotal.toFixed(2)} ₽</b><br>`;
    }
...
/* === Calculator: Штакетник (категория 273) === */

// ===== АДМИНКА =====
add_action('add_meta_boxes', function() {
    add_meta_box('shtaketnik_prices', 'Настройки формы верха (Штакетник)', 'render_shtaketnik_meta_box', 'product', 'side', 'default');
});

function render_shtaketnik_meta_box($post) {
    if(!has_term(273, 'product_cat', $post->ID)) return;
    $r = get_post_meta($post->ID, '_shape_price_round', true);
    $t = get_post_meta($post->ID, '_shape_price_triangle', true);
    $f = get_post_meta($post->ID, '_shape_price_flat', true);
    echo '<label>Полукруг:</label><input type="number" name="shape_price_round" value="'.esc_attr($r).'" step="1" style="width:100%">';
    echo '<label>Треугольник:</label><input type="number" name="shape_price_triangle" value="'.esc_attr($t).'" step="1" style="width:100%">';
    echo '<label>Прямой спил:</label><input type="number" name="shape_price_flat" value="'.esc_attr($f).'" step="1" style="width:100%">';
}

add_action('save_post', function($post_id){
    if(get_post_type($post_id)!=='product' || !has_term(273, 'product_cat', $post_id)) return;
    foreach(['round','triangle','flat'] as $k){
        if(isset($_POST["shape_price_$k"]))
            update_post_meta($post_id,"_shape_price_$k",floatval($_POST["shape_price_$k"]));
    }
});
...
add_action('wp_footer', function () {
    ...
    // Проверяем категорию штакетника
    $is_shtaketnik = has_term(273, 'product_cat', $product_id);
    
    if (!$is_shtaketnik) return;
    
    // Получаем цены за верхушку
    $shape_prices = array(
        'round' => floatval(get_post_meta($product_id, '_shape_price_round', true)) ?: 0,
        'triangle' => floatval(get_post_meta($product_id, '_shape_price_triangle', true)) ?: 0,
        'flat' => floatval(get_post_meta($product_id, '_shape_price_flat', true)) ?: 0,
    );
    ...
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        // Получаем базовую цену и цены за формы верхушки
        const basePrice = <?php echo $base_price; ?>;
        const shapePrices = <?php echo json_encode($shape_prices); ?>;
        ...
        // ===== СОЗДАЕМ БЛОК ВЫБОРА ВЕРХУШКИ =====
        const shapeCalcBlock = document.createElement('div');
        shapeCalcBlock.id = 'shtaketnik-shape-calc';
        shapeCalcBlock.style.cssText = 'margin: 15px 0; padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;';
        
        let shapeHTML = '<h4 style="margin-top: 0;">Выберите форму верхушки штакетника:</h4>';
        shapeHTML += '<div style="display: flex; gap: 15px; flex-wrap: wrap; align-items: center;">';
        
        // Чекбокс для каждой формы
        shapeHTML += `
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input type="radio" name="shtaketnik_shape" value="flat" style="cursor: pointer;" />
                <span>Прямой спил</span>
                <span style="color: #666; font-size: 0.9em;">${shapePrices.flat > 0 ? '(+' + shapePrices.flat + '₽)' : ''}</span>
            </label>
            
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input type="radio" name="shtaketnik_shape" value="round" style="cursor: pointer;" />
                <span>Полукруг</span>
                <span style="color: #666; font-size: 0.9em;">${shapePrices.round > 0 ? '(+' + shapePrices.round + '₽)' : ''}</span>
            </label>
            
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input type="radio" name="shtaketnik_shape" value="triangle" style="cursor: pointer;" />
                <span>Треугольник</span>
                <span style="color: #666; font-size: 0.9em;">${shapePrices.triangle > 0 ? '(+' + shapePrices.triangle + '₽)' : ''}</span>
            </label>
        `;
        
        shapeHTML += '</div><div id="shape_price_display" style="margin-top: 10px; font-size: 1.1em; font-weight: bold; color: #2a2a2a;"></div>';
        shapeCalcBlock.innerHTML = shapeHTML;
        
        // Вставляем блок выбора перед кнопкой "Добавить в корзину"
        const addToCartBtn = form.querySelector('.single_add_to_cart_button') || 
                            form.querySelector('button[type="submit"]');
        if (addToCartBtn) {
            addToCartBtn.parentElement.insertBefore(shapeCalcBlock, addToCartBtn);
        } else {
            form.appendChild(shapeCalcBlock);
        }
        
        // Получаем элементы формы
        const shapeInputs = form.querySelectorAll('input[name="shtaketnik_shape"]');
        const shapePriceDisplay = document.getElementById('shape_price_display');
        
        // Функция обновления цены при выборе формы верхушки
        function updateShapePrice() {
            const selectedShape = form.querySelector('input[name="shtaketnik_shape"]:checked');
            const shapeValue = selectedShape?.value || '';
            const shapePrice = shapeValue ? shapePrices[shapeValue] : 0;
            const quantity = quantityInput ? (parseInt(quantityInput.value) || 1) : 1;
            
            // Удаляем предыдущее скрытое поле
            const oldHiddenField = form.querySelector('input[name="custom_shtaketnik_shape"]');
            if (oldHiddenField) oldHiddenField.remove();
            
            const oldHiddenPrice = form.querySelector('input[name="custom_shtaketnik_shape_price"]');
            if (oldHiddenPrice) oldHiddenPrice.remove();
            
            // Добавляем новые скрытые поля
            if (shapeValue) {
                const hiddenShape = document.createElement('input');
                hiddenShape.type = 'hidden';
                hiddenShape.name = 'custom_shtaketnik_shape';
                hiddenShape.value = shapeValue;
                form.appendChild(hiddenShape);
                
                const hiddenPrice = document.createElement('input');
                hiddenPrice.type = 'hidden';
                hiddenPrice.name = 'custom_shtaketnik_shape_price';
                hiddenPrice.value = (shapePrice * quantity).toFixed(2);
                form.appendChild(hiddenPrice);
                
                // Обновляем отображение цены
                shapePriceDisplay.innerHTML = `
                    Цена за форму верхушки: <b>${shapePrice.toFixed(2)} ₽</b> за 1 шт<br>
                    <span style="color: #999; font-size: 0.95em;">Всего: <b>${(shapePrice * quantity).toFixed(2)} ₽</b> за ${quantity} шт</span>
                `;
            } else {
                shapePriceDisplay.innerHTML = '';
            }
        ...
        // Слушаем изменения формы верхушки
        shapeInputs.forEach(input => {
            input.addEventListener('change', updateShapePrice);
        });
        
        // Слушаем изменение количества
        if (quantityInput) {
            quantityInput.addEventListener('input', updateShapePrice);
            quantityInput.addEventListener('change', updateShapePrice);
        }
        
        // Слушаем клик по кнопке "Добавить в корзину"
        const addBtn = form.querySelector('.single_add_to_cart_button') ||
                      form.querySelector('button[type="submit"][name="add-to-cart"]') ||
                      form.querySelector('button[type="submit"]');
        ...
        if (addBtn) {
            addBtn.addEventListener('click', function(e) {
                const selectedShape = form.querySelector('input[name="shtaketnik_shape"]:checked');
                if (!selectedShape) {
                    // Опционально: требуем выбор формы
                    // e.preventDefault();
                    // alert('Пожалуйста, выберите форму верхушки штакетника');
                    // return false;
                }
            });
        }
        
        console.log('✓ Shtaketnik shape calculator initialized');
    });
    </script>
    <?php
}, 20);

// ===== ОБРАБОТКА В КОРЗИНЕ И ЗАКАЗЕ =====

// Добавляем данные верхушки в элемент корзины
add_filter('woocommerce_add_cart_item_data', function($cart_item_data, $product_id) {
    if (has_term(273, 'product_cat', $product_id)) {
        if (isset($_POST['custom_shtaketnik_shape'])) {
            $cart_item_data['shtaketnik_shape'] = sanitize_text_field($_POST['custom_shtaketnik_shape']);
        }
        if (isset($_POST['custom_shtaketnik_shape_price'])) {
            $cart_item_data['shtaketnik_shape_price'] = floatval($_POST['custom_shtaketnik_shape_price']);
        }
    }
    return $cart_item_data;
}, 10, 2);

// Отображаем выбранную форму верхушки в корзине
add_filter('woocommerce_get_item_data', function($item_data, $cart_item) {
    if (isset($cart_item['shtaketnik_shape'])) {
        $shape_labels = array(
            'flat' => 'Прямой спил',
            'round' => 'Полукруг',
        ...
        $shape = $cart_item['shtaketnik_shape'];
        ...
        if (isset($cart_item['shtaketnik_shape_price'])) {
            $item_data[] = array(
                'key' => 'Цена верхушки',
                'value' => wc_price($cart_item['shtaketnik_shape_price'])
            );
        }
    }
    return $item_data;
...
// Добавляем цену верхушки к итоговой цене товара в корзине
add_action('woocommerce_before_calculate_totals', function($cart) {
    if (is_admin()) return;
    
    foreach ($cart->get_cart() as $cart_item) {
        if (isset($cart_item['shtaketnik_shape_price']) && $cart_item['shtaketnik_shape_price'] > 0) {
            // Получаем базовую цену товара
            $product = $cart_item['data'];
            $base_price = floatval($product->get_price());
            $shape_price = floatval($cart_item['shtaketnik_shape_price']) / $cart_item['quantity'];
            
            // Устанавливаем новую цену (базовая + верхушка за единицу)
            $product->set_price($base_price + $shape_price);
        }
    }
});

// Сохраняем данные верхушки в заказ
add_action('woocommerce_checkout_create_order_line_item', function($item, $cart_item_key, $cart_item) {
    if (isset($cart_item['shtaketnik_shape'])) {
        $item->add_meta_data('_shtaketnik_shape', $cart_item['shtaketnik_shape']);
    }
    if (isset($cart_item['shtaketnik_shape_price'])) {
        $item->add_meta_data('_shtaketnik_shape_price', $cart_item['shtaketnik_shape_price']);
    }
}, 10, 3);

// Отображаем форму верхушки в заказе (админка)
add_action('woocommerce_admin_order_item_values', function($product, $item, $item_id) {
    $shape = $item->get_meta('_shtaketnik_shape');
    $shape_price = $item->get_meta('_shtaketnik_shape_price');
    ...
    if ($shape) {
        ...
        echo '<strong>Форма верхушки:</strong> ' . esc_html($shape_labels[$shape] ?? $shape);
        if ($shape_price) {
            echo '<br><strong>Цена верхушки:</strong> ' . wc_price($shape_price);
        }
        echo '</div>';
    }
}, 10, 3);

// Отображаем форму верхушки на странице заказа (фронт)
add_filter('woocommerce_order_item_get_formatted_meta_data', function($formatted_meta, $item) {
    ...
    if ($shape) {
        ...
        // Удаляем дублирующиеся метаданные
        $formatted_meta = array_filter($formatted_meta, function($meta) {
            return $meta->key !== '_shtaketnik_shape' && $meta->key !== '_shtaketnik_shape_price';
        });
        
        // Добавляем отформатированные данные
        $formatted_meta[] = (object)[
            'key' => 'Форма верхушки',
            'value' => $shape_labels[$shape] ?? $shape,
            'display_key' => 'Форма верхушки',
            'display_value' => $shape_labels[$shape] ?? $shape
        ];
        
        if ($item->get_meta('_shtaketnik_shape_price')) {
            $formatted_meta[] = (object)[
                'key' => 'Цена верхушки',
                'value' => wc_price($item->get_meta('_shtaketnik_shape_price')),
                'display_key' => 'Цена верхушки',
                'display_value' => wc_price($item->get_meta('_shtaketnik_shape_price'))
            ];
        }
    }
